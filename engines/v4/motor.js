// ══════════════════════════════════════════════════════════════
//  AtOhmEter Core V4 — Motor Cosserat Bifásico
...
export function createEngine(N, renderVolume, phaseData, params) {
    ...
    // Tiempo interno para bombeo periódico
    let _pump_t = 0.0;
    ...
    function step() {
        ...
        if (P._seed === 'nucleo_bombeante') {
            _pump_t += dt;
            const _amp = (P.NUCLEUS_AMP||0.8) * Math.sin(2*Math.PI*(P.PUMP_FREQ||0.05)*_pump_t);
            const _c = N>>1, _R = Math.max(2,Math.round(P.NUCLEUS_R||3));
            const _si = _R+1, _so = _R+3;
            for (let x=0;x<N;x++) for (let y=0;y<N;y++) for (let z=0;z<N;z++) {
                const dx=x-_c,dy=y-_c,dz=z-_c;
                const r=Math.sqrt(dx*dx+dy*dy+dz*dz)+1e-6;
                const i=idx(x,y,z);
                if (r<=_R) {
                    phi[i]=dz>=0?1.0:-1.0; phi_prev[i]=phi[i];
                    thx[i]=Math.PI*0.5*(dy/(_R+1e-6));
                    thy[i]=-Math.PI*0.5*(dx/(_R+1e-6));
                    thz[i]=Math.PI*0.3;
                } else if (r>=_si&&r<=_so) {
                    ux_v[i]+=_amp*(dx/r)*dt;
                    uy_v[i]+=_amp*(dy/r)*dt;
    ...
    function initSeed(name) {
        ...
        else if (name === 'nucleo_bombeante') { P._seed = 'nucleo_bombeante'; seedNucleoBombeante(); }
        else                            initGrumo();
        refresh();
    }
    ...
    function seedNucleoBombeante() {
        const c = N >> 1;
        const R = Math.max(2, Math.round(P.NUCLEUS_R || 3));
        for (let x=0;x<N;x++) for (let y=0;y<N;y++) for (let z=0;z<N;z++) {
            const i = idx(x,y,z);
            const dx=x-c, dy=y-c, dz=z-c;
            const r = Math.sqrt(dx*dx+dy*dy+dz*dz)+1e-6;
            if (r <= R) {
                phi[i] = dz >= 0 ? 1.0 : -1.0;
                thx[i] =  Math.PI*0.5*(dy/(R+1e-6));
                thy[i] = -Math.PI*0.5*(dx/(R+1e-6));
                thz[i] =  Math.PI*0.3;
            } else {
                phi[i] = r < N*0.45 ? 0.0 : 1.0;
                thx[i]=0; thy[i]=0; thz[i]=0;
                ux[i]=0; uy[i]=0; uz[i]=0;
            }
            phi_prev[i]=phi[i];
