...
export function createEngine(N, renderVolume, phaseData, texture3D, texturePhase) {
    ...
    let P = {
        ...
        // Luces RGB
        LIGHT_R_X:   -0.7,   // posición X de la luz roja arriba [-1,1]
        LIGHT_R_X2:  -0.7,   // posición X de la luz roja abajo [-1,1]
        LIGHT_G_X:    0.0,   // posición X de la luz verde arriba
        LIGHT_G_X2:   0.0,   // posición X de la luz verde abajo
        LIGHT_B_X:    0.7,   // posición X de la luz azul arriba
        LIGHT_B_X2:   0.7,   // posición X de la luz azul abajo
    ...
    // X arriba (LIGHT_*_X) y X abajo (LIGHT_*_X2) definen la
    // orientación del haz — interpola la posición X en cada fila Y
    function propagateLights() {
        const c = N >> 1;
        const r = Math.max(2, Math.floor(N * P.LIGHT_SPREAD * 0.15));

        const lamps = [
            { F: light_r, xTop: P.LIGHT_R_X, xBot: P.LIGHT_R_X2 },
            { F: light_g, xTop: P.LIGHT_G_X, xBot: P.LIGHT_G_X2 },
            { F: light_b, xTop: P.LIGHT_B_X, xBot: P.LIGHT_B_X2 },
        ];

        for (const lamp of lamps) {
            // Inyectar luz en el techo (Y = N-1) usando xTop
            const gxTop = Math.floor((lamp.xTop * 0.5 + 0.5) * (N-1));
            for (let dx=-r; dx<=r; dx++) for (let dz=-r; dz<=r; dz++) {
                if (dx*dx + dz*dz > r*r) continue;
                const px = Math.max(0, Math.min(N-1, gxTop + dx));
                const pz = Math.max(0, Math.min(N-1, c    + dz));
                lamp.F[idx(px, N-1, pz)] = 1.0;
            }

            // Difundir hacia abajo con scattering del fluido
            // En cada fila Y, el centro del haz se desplaza según la
            // interpolación lineal xTop → xBot
            for (let y = N-2; y >= 0; y--) {
                // t=1 en techo, t=0 en suelo → offset X del haz en esta fila
                const t      = y / (N - 1);
                const xBeam  = lamp.xTop * t + lamp.xBot * (1 - t);
                const xAbove = lamp.xTop * ((y+1) / (N - 1)) + lamp.xBot * (1 - (y+1) / (N - 1));
                // Desplazamiento en celdas entre esta fila y la de arriba
                const shift  = Math.round(
                    ((xBeam - xAbove) * 0.5) * (N - 1)
                );

                for (let x = 0; x < N; x++) for (let z = 0; z < N; z++) {
                    const i      = idx(x, y, z);
                    const srcX   = Math.max(0, Math.min(N-1, x - shift));
                    const above  = lamp.F[idx(srcX, y+1, z)];
