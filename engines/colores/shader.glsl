...
// Cáustics procedurales — simulan la concentración de luz en el fluido
float causticPattern(vec3 pos, float t){
    vec3 p = pos * 5.0;
    float c = sin(p.x + t*0.7)*sin(p.z + t*0.5)
            + sin(p.x*1.4 - p.z*0.8 + t*0.9)
            + sin(p.y*1.2 + p.x*0.6 + t*0.6);
...
void main(){
    ...
    for(int i=0;i<128;i++){
        ...
        // ── Cáustics ──────────────────────────────────────────
        float caust = causticPattern(pos, uTime);
        col += baseCol * caust * 0.4 * density;
    ...
    // Borde del cubo — reflexión interna total
    // Las aristas del cubo brillan iridiscentes
