...
export function createEngine(N, renderVolume, phaseData, texture3D, texturePhase) {
    ...
    let P = {
        ...
        TOPO:       0,     // 0 = cubo (toroide), 1 = esfera, 2 = wormhole. Topología de ŚN1E.
    ...
    // ── Topología del dominio (idea de ŚN1E, traducida a campo 3D) ──
    // En ŚN1E la topología gobierna el movimiento de partículas (toroide,
    // círculo, círculo+wormhole). Aquí no hay partículas que viajen: el
    // campo es función pura de la coordenada. Así que la topología se vuelve
    // un mapeo de coordenadas + máscara de dominio ANTES del muestreo lógico:
    //   0 · Cubo      — toroide; el campo llena el cubo entero.
    //   1 · Esfera    — el campo solo vive dentro de la esfera inscrita.
    //   2 · Wormhole  — esfera con el núcleo COSIDO al borde: el patrón que
    //                   viviría en el centro se remapea radialmente a la
    //                   cáscara (geometría que pasa de un lado al otro).
    // Devuelve null si la celda está fuera del dominio (→ campo 0).
    // Si no, devuelve las coordenadas normalizadas [0,1) ya transformadas.
    const WORM_CORE = 0.28; // radio del núcleo que se cose al borde (modo 2)
    function topoMap(x, y, z) {
        const topo = Math.round(P.TOPO);
        // normalizar a [0,1) centrado
        let nx = x / N, ny = y / N, nz = z / N;
        if (topo === 0) {
            return [nx, ny, nz]; // cubo/toroide — sin recorte
        }
        // esfera inscrita: radio 0.5 desde el centro (0.5,0.5,0.5)
        const dx = nx - 0.5, dy = ny - 0.5, dz = nz - 0.5;
        const r = Math.sqrt(dx*dx + dy*dy + dz*dz);
        if (r > 0.5) return null; // fuera de la esfera → vacío
        if (topo === 1) {
            return [nx, ny, nz]; // esfera — campo normal dentro de la bola
        }
        // topo === 2 — wormhole: el núcleo interior se cose a la cáscara.
        // Reescalamos el radio: [0, 0.5] → [WORM_CORE, 0.5] empujando el
        // centro hacia afuera, y lo que cae bajo WORM_CORE se "teletransporta"
        // continuando por el otro extremo del radio (geometría que cruza).
        if (r < 1e-6) return null; // singularidad exacta del centro → vacío
        const dirx = dx / r, diry = dy / r, dirz = dz / r;
        let rr;
        if (r < WORM_CORE) {
            // núcleo: cosido al borde — mapea [0,WORM_CORE) → [0.5, WORM_CORE)
            // invertido, de modo que el centro toca la superficie.
            const tcore = r / WORM_CORE;           // 0..1
            rr = 0.5 - tcore * (0.5 - WORM_CORE);  // 0.5 → WORM_CORE
        } else {
            rr = r; // cáscara exterior: sin cambio
        }
        return [0.5 + dirx*rr, 0.5 + diry*rr, 0.5 + dirz*rr];
    }
    ...
    function computeField(ruleMask) {
        ...
        for (let x=0;x<N;x++) for (let y=0;y<N;y++) for (let z=0;z<N;z++) {
            const i = idx(x,y,z);
            const m = topoMap(x, y, z);
            if (m === null) { field[i] = 0; continue; } // fuera del dominio
            // discretizar coordenada transformada [0,1) → entero [0, 2^bits)
            const wx = Math.min(p-1, (m[0] * p) | 0);
            const wy = Math.min(p-1, (m[1] * p) | 0);
            const wz = Math.min(p-1, (m[2] * p) | 0);
            field[i] = logicVoltage(wx, wy, wz, ruleMask, bits);
        }
    }
    ...
    function seedTerreno() {
        mode = 'B';
        computeField(P.RULE & 15);
        // sembrar polvo uniforme que luego caerá al patrón — solo en el dominio
        for (let x=0;x<N;x++) for (let y=0;y<N;y++) for (let z=0;z<N;z++) {
            const i = idx(x,y,z);
            dust[i] = (topoMap(x,y,z) === null) ? 0 : 0.5 + 0.1*Math.random();
        }
    }
    function seedDeriva() {
        mode = 'C';
    ...
    return {
        ...
        savePrev() {},
        applyParams(p) {
            const oldRule = P.RULE, oldGeom = P.GEOM, oldBits = P.BITS, oldTopo = P.TOPO;
            Object.assign(P, p);
            // Si cambió algo que define el paisaje, recalcular (en A y B;
            // en C el step lo recalcula solo cada frame).
            if (mode !== 'C' && (P.RULE !== oldRule || P.GEOM !== oldGeom || P.BITS !== oldBits || P.TOPO !== oldTopo)) {
                computeField(P.RULE & 15);
                refresh();
            }
        },
