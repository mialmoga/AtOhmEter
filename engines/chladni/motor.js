...
export function createEngine(N, renderVolume, phaseData, texture3D, texturePhase) {
    ...
    //  Fase    = VIBRACIÓN (color: nodo quieto → antinodo vibrante)
    // ══════════════════════════════════════════════════════════
    function refresh() {
        ...
        for (let i=0;i<T;i++) {
            ...
            // Color = vibración local normalizada [0,1]. El shader la mapea:
            //   vib baja (nodo, quieto) → frío/azul
            //   vib alta (antinodo)     → cálido/rojo
            phaseData[i] = Math.min(1.0, vib[i] / vmax);
        }
