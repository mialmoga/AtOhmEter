"""
AtOhmEter — engines/v3core/compute.py
Motor Cosserat Bifásico V3 — NumPy vectorizado
...
def createEngine(N, render_volume, phase_data):
    ...
    def step():
        ...
        if P['_seed'] == 'nucleo_bombeante':
            ...
            mask_in = r <= R
            phi[mask_in]  = np.where(dz[mask_in] >= 0, 1.0, -1.0)
            phi_prev[mask_in] = phi[mask_in]
            thx[mask_in]  =  np.pi*0.5*(dy/(R+1e-6))[mask_in]
            thy[mask_in]  = -np.pi*0.5*(dx/(R+1e-6))[mask_in]
            thz[mask_in]  =  np.pi*0.3

            # Shell del núcleo — inyectar velocidad radial
            mask_sh = (r >= si) & (r <= so)
            ux_v[mask_sh] += amp*(dx/r)[mask_sh]*dt
            uy_v[mask_sh] += amp*(dy/r)[mask_sh]*dt
            uz_v[mask_sh] += amp*(dz/r)[mask_sh]*dt
    ...
    def seed_bessel(bc, l):
        ...
        if bc == 'dirichlet':
            p = np.where(r > 1.0, 0.0, np.where(r < 1e-8, 1.0, np.sin(root*r)/(root*r)))
        else:
            p = np.where(r > 1.0, 0.0, np.cos(root*r)*np.exp(-r**2*1.5))
        phi[:] = p; phi_prev[:] = p
        mask = (r > 0.01) & (r < 1.0)
        thx[:] = np.where(mask, -(fx/r)*phi*0.3, 0)
        thy[:] = np.where(mask, -(fy/r)*phi*0.3, 0)
        thz[:] = np.where(mask, -(fz/r)*phi*0.3, 0)
        phi_seed[:] = phi
    ...
    def inject(name, data=None):
        ...
        if name == 'perturbacion':
            ...
            phi[mask]      *= -1
            phi_prev[mask] *= -1
            thx[mask] += (np.random.rand(mask.sum())-0.5)*1.0
            thy[mask] += (np.random.rand(mask.sum())-0.5)*1.0
